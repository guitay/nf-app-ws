package com.nf.app.bean.pojo;

import com.nf.app.bean.base.Model;

public class APP_ZH  extends Model{

	private String zhdh;
	private String zhye;
	private String zhbhje;
	private String jxdxlx;
	private String ghxx;
	private String ghbl;
	private String ghsj;
	private String khrq;
	private String dqrq;
	private String kmh;
	private String kmmc;
	private String xszhdh;
	
	
	
	
	public String getXszhdh() {
		return xszhdh;
	}
	public void setXszhdh(String xszhdh) {
		this.xszhdh = xszhdh;
	}
	public String getGhxx() {
		return ghxx;
	}
	public void setGhxx(String ghxx) {
		this.ghxx = ghxx;
	}
	public String getGhbl() {
		return ghbl;
	}
	public void setGhbl(String ghbl) {
		this.ghbl = ghbl;
	}
	public String getGhsj() {
		return ghsj;
	}
	public void setGhsj(String ghsj) {
		this.ghsj = ghsj;
	}
	public String getKhrq() {
		return khrq;
	}
	public void setKhrq(String khrq) {
		this.khrq = khrq;
	}
	public String getDqrq() {
		return dqrq;
	}
	public void setDqrq(String dqrq) {
		this.dqrq = dqrq;
	}
	public String getKmh() {
		return kmh;
	}
	public void setKmh(String kmh) {
		this.kmh = kmh;
	}
	public String getKmmc() {
		return kmmc;
	}
	public void setKmmc(String kmmc) {
		this.kmmc = kmmc;
	}
	public String getJxdxlx() {
		return jxdxlx;
	}
	public void setJxdxlx(String jxdxlx) {
		this.jxdxlx = jxdxlx;
	}
	public String getZhdh() {
		return zhdh;
	}
	public void setZhdh(String zhdh) {
		this.zhdh = zhdh;
	}
	public String getZhye() {
		return zhye;
	}
	public void setZhye(String zhye) {
		this.zhye = zhye;
	}
	public String getZhbhje() {
		return zhbhje;
	}
	public void setZhbhje(String zhbhje) {
		this.zhbhje = zhbhje;
	}
	
	
}
