package com.nf.app.bean.pojo;

import java.io.Serializable;


public class M_KHB implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String yhbs;
	private String yhmc;
	private String yhfl;
	private String sftsdx;
	private String sftsyj;
	private String zcrq;
	private String yhid ;
	private String hydh;
	private String hymc;
	private String dlmc;
	private String dlmm;
	private String sfscdl;
	private String sjhm;
	private String yjdz;
	private String wxh;
	
	
	public String getYhid() {
		return yhid;
	}
	public void setYhid(String yhid) {
		this.yhid = yhid;
	}
	public String getHydh() {
		return hydh;
	}
	public void setHydh(String hydh) {
		this.hydh = hydh;
	}
	public String getDlmc() {
		return dlmc;
	}
	public void setDlmc(String dlmc) {
		this.dlmc = dlmc;
	}
	public String getDlmm() {
		return dlmm;
	}
	public void setDlmm(String dlmm) {
		this.dlmm = dlmm;
	}
	public String getSfscdl() {
		return sfscdl;
	}
	public void setSfscdl(String sfscdl) {
		this.sfscdl = sfscdl;
	}
	public String getSjhm() {
		return sjhm;
	}
	public void setSjhm(String sjhm) {
		this.sjhm = sjhm;
	}
	public String getYjdz() {
		return yjdz;
	}
	public void setYjdz(String yjdz) {
		this.yjdz = yjdz;
	}
	public String getWxh() {
		return wxh;
	}
	public void setWxh(String wxh) {
		this.wxh = wxh;
	}
	public String getYhbs() {
		return yhbs;
	}
	public void setYhbs(String yhbs) {
		this.yhbs = yhbs;
	}
	public String getYhmc() {
		return yhmc;
	}
	public void setYhmc(String yhmc) {
		this.yhmc = yhmc;
	}
	public String getYhfl() {
		return yhfl;
	}
	public void setYhfl(String yhfl) {
		this.yhfl = yhfl;
	}
	public String getSftsdx() {
		return sftsdx;
	}
	public void setSftsdx(String sftsdx) {
		this.sftsdx = sftsdx;
	}
	public String getSftsyj() {
		return sftsyj;
	}
	public void setSftsyj(String sftsyj) {
		this.sftsyj = sftsyj;
	}
	public String getZcrq() {
		return zcrq;
	}
	public void setZcrq(String zcrq) {
		this.zcrq = zcrq;
	}
	public String getHymc() {
		return hymc;
	}
	public void setHymc(String hymc) {
		this.hymc = hymc;
	}
	
	
	
}
